...
%$Author: V $
%$Id: substrate_update.m 106 2017-06-27 12:45:13Z V $
%$HeadURL: https://131.180.60.193/svn/ELV/branches/V0123/main/substrate_update.m $
%
...
%
%170623
%	V. Removed repmat to increase efficiency
%	V. Reordered 'if no cell change' to first check this and avoid one if clause most of the times

function [msk_new,Ls_new,fIk,detaLa]=substrate_update_2(Mak,msk,Ls,La_old,La,etab_old,etab,qbk,input,fid_log,kt)
...
Fak=Mak./La_old; %volume fractions at the active layer [-]; [nef x nx double]
...
for kx=1:nx
    if agr_idx(kx) %aggrading nodes
        ...
        if ec(kx)~=1 %more than 1 substrate layer
            ...
            if cj(kx)==0 %there is no cell jump

                Ls_new(1,kx,1)=Ls1_temp_new(1,kx,1)-cj(kx)*ThUnLyr; %top layer
                
                msk_new(:,kx,1)=msk(:,kx,1)+fIk(:,kx).*(Ls_new(1,kx,1)-Ls(1,kx,1)); %top layer
                
            else %there is at least a cell jump
                ...
                msk_new(:,kx,1)                =fIk(:,kx).*Ls_new(1,kx,1);
                msk_new(:,kx,1+cj(kx))         =msk_new(:,kx,1+cj(kx))-fIk(:,kx).*Ls(1,kx,1);
                msk_new(:,kx,2:cj(kx)+1)       =msk_new(:,kx,2:cj(kx)+1)+fIk(:,kx)*ThUnLyr;
            ...
            if ec(kx)+cj(kx)<=nsl %the end cell has thickness 0; add one layer
                ...
                msk_new(:,kx,2:cj(kx))    =fIk(:,kx)*ThUnLyr;
                ...
                msk_new(:,kx,2:nsl-1)=fIk(:,kx)*ThUnLyr;
        ...
        if cj(kx)==0 %there is no cell jump
            ...
            msk_new(:,kx,1)=msk(:,kx,1)-(Ls(1,kx,1)-Ls_new(1,kx,1))/Ls(1,kx,1).*msk(:,kx,1);
            ...
            %mass
            msk_new(:,kx,1)                  =msk(:,kx,1+cj(kx))-(Ls(1,kx,1+cj(kx))-Ls_new(1,kx,1))/Ls(1,kx,1+cj(kx)).*msk(:,kx,1+cj(kx));

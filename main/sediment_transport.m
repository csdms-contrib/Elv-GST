...
%$Author: V $
%$Id: sediment_transport.m 106 2017-06-27 12:45:13Z V $
%$HeadURL: https://131.180.60.193/svn/ELV/branches/V0123/main/sediment_transport.m $
%
...
switch flg.sed_trans
    case 1 %MPM48
        ...
        Qbk=a_mpm.*sqrt(cnt.g*cnt.R*dk.^3)./(1-cnt.p).*(thetak-xik.*theta_c).^b_mpm; %MPM relation
    ...
    case 2 %EH67
        ...
        Qbk=m_eh*cf.^(3/2)/cnt.g^2/cnt.R^2/(1-cnt.p)./dk.*(u.^n_eh); %EH relation
    case 3 %AM72
        a_am=sed_trans_param(1);
        theta_c=sed_trans_param(2);
        no_trans_idx=(thetak-xik.*theta_c)<0; %indexes of fractions below threshold
        Qbk=a_am.*sqrt(cnt.g*cnt.R*dk.^3)./(1-cnt.p).*(thetak-xik.*theta_c).*(sqrt(thetak)-sqrt(xik.*theta_c));
        Qbk(no_trans_idx)=0; %the transport capacity of those fractions below threshold is 0
    case 4 %WC03 
    ...
    case 5 %Generalized load relation
        ...
        Qbk = (dk/D_ref).^r.*G.*1./dk.^w.*(q./h).^(2*w+3);
    otherwise 
